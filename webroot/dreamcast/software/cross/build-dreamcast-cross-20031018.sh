...
#
# changelog
#  * 2003 10/17 update binutils, gcc, newlib
...
#  * This script works on Mac OS X (thanks HISAMATSU tatsuaki)
...
binutils_targz="binutils-2.14.tar.gz"
gcc_version="3.3.1"
gcc_targz="gcc-3.3.1.tar.gz"
newlib_version="1.11.0"
newlib_targz="newlib-1.11.0.tar.gz"

[ -x $(which make) ] && local_make=$(which make)
[ -x $(which gmake) ] && local_make=$(which gmake)
...
build_gcc () {
    ...
    # pre-mkdir to avoid gcc-3.3.1 specific bugs
    mkdir -p $prefix/lib/gcc-lib/sh-elf/$gcc_version/ml

    line="$line $src_dir/gcc-${gcc_version}/configure $configure_args"
    line="$line &&"
#    line="$line ${local_make} all install CPPFLAGS='-no-cpp-precomp' ASFLAGS+='-little'"
    line="$line ${local_make} all install ASFLAGS+='-little'"
    do_command "$line"
}
